...
<head>
  ...
  <title>CVP | Inscripción Cursos</title>
  ...
  <section id ="wrap">
      <section id="main">
        ...
        <section id="contenido">
            <div id="consulta">
                <h3>Inscripción Cursos</h3>
                <section id="form-producto">
                <form id="forml-prod" name="forml-prod" method="post" action="../estu/inscribircurso.php">
                    <p>Seleccione el curso al que desea inscribirse:</p>
                    <?php 
                        $conexion = pg_connect("host='localhost' dbname='cursos_rbc' user='postgres' password='%froac$'");
                        if($conexion==FALSE){
                            header("Location:../error/error-conex.html");
                        }
                        $sql = "SELECT * FROM curso WHERE id_curso NOT IN (SELECT curso.id_curso FROM curso, cursoEstudiante WHERE curso.id_curso = cursoEstudiante.id_curso AND id_estudiante='$nombre')";
                        $result = pg_query($sql);
                        ...
                        while($row= pg_fetch_array($result))	{
                         echo "<OPTION value=$row[id_curso]>$row[descripcion]</OPTION>";
                        }
                    ...
                    <br/>
                    <br/>
                    <button type="submit" name="mostrar" id="mostrar">Inscribirse</button>
                    </form>
                    </section>
                            <hr/>
                    </div>
